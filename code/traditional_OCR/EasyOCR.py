...
def ocr_pdf(pdf_path, lang="en", dpi=350, first_page=None, last_page=None):
    """
    Convert PDF to images using PyMuPDF and perform OCR with EasyOCR
    """
    # Initialize EasyOCR reader
    reader = easyocr.Reader([lang], gpu=False)  # Set gpu=True if you have CUDA
    ...
    for page_num in range(start_page, end_page):
        ...
        print(f"[debug] EasyOCR'd Page {page_num + 1}: {len(page_text)} chars, {len(results)} paragraphs")
...
def highlight_differences(ocr_text, ground_truth, matcher):
    """
    Generate a text with highlights showing character-level differences between OCR and ground truth
    """
    result = []
    for tag, i1, i2, j1, j2 in matcher.get_opcodes():
        if tag == 'equal':
...
def main():
    ...
    ocr_pages = ocr_pdf(pdf_file, lang="en")
    ...
    with open("a01-00_easyocr.txt", "w", encoding="utf-8") as f:
        f.write(full_ocr_text)
    print("\nSaved full EasyOCR results to a01-00_easyocr.txt")

    print("\n" + "="*60)
    print("EASYOCR ACCURACY ANALYSIS REPORT")
    print("="*60)
    ...
    # Show character-level differences for the first page
    print("\n" + "="*60)
    print("CHARACTER-LEVEL DIFFERENCES (Page 1)")
    print("="*60)
    char_diff_text = highlight_differences(
        page_char_metrics[0]['ocr_clean'],
        page_char_metrics[0]['gt_clean'],
        page_char_metrics[0]['matcher']
    )
    print(char_diff_text[:1000] + "..." if len(char_diff_text) > 1000 else char_diff_text)

    # Save detailed comparison to file
    with open("easyocr_comparison_report.txt", "w", encoding="utf-8") as f:
        f.write("EASYOCR ACCURACY ANALYSIS REPORT\n")
        f.write("="*60 + "\n")
        ...
        f.write("CHARACTER-LEVEL DIFFERENCES\n")
        f.write("="*60 + "\n")
        for i, char_metrics in enumerate(page_char_metrics, 1):
            f.write(f"\n--- Page {i} Character Differences ---\n")
            char_diff = highlight_differences(
                char_metrics['ocr_clean'],
                char_metrics['gt_clean'],
                char_metrics['matcher']
            )
            f.write(char_diff + "\n\n")

    print("\nSaved detailed comparison report to easyocr_comparison_report.txt")
